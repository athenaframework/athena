#!/usr/bin/env python3
"""ATHENA Bot + Dashboard Unified Launcher (Cross-Platform)
...
def main():
    print("\n" + "="*60)
    print("ATHENA: Bot + Dashboard Unified Launcher")
    print("="*60 + "\n")
    ...
    if platform.system() == "Windows":
        subprocess.Popen(f'start "ATHENA Backend" cmd /k {backend_cmd}', shell=True)
    else:
    ...
    dashboard_dir = Path("ATHENA") if Path("ATHENA").exists() else Path("dashboard")
    ...
    if platform.system() == "Windows":
        subprocess.Popen(f'start "ATHENA Dashboard" cmd /k {dashboard_cmd}', shell=True)
    else:
    ...
    if platform.system() == "Windows":
        subprocess.Popen(f'start "ATHENA Bot" cmd /k {bot_cmd}', shell=True)
    else:
