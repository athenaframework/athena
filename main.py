"""main.py
ATHENA - Unified CLI entry point.
Usage examples:
  python main.py --help
...
def banner():
    console.print(
        Panel.fit(
            "[bold cyan]ATHENA[/bold cyan]  •  [bold magenta]ATHENA Swarm[/bold magenta]\n"
...
def cmd_ATHENA(args):
    banner()
    daemon = ATHENADaemon(project_root=str(ROOT))
    if args.once:
        console.print("[yellow]Running single ATHENA scan...[/yellow]")
        daemon.run_once()
    else:
        console.print("[bold red]Starting ATHENA autonomous daemon (Ctrl+C to stop)[/bold red]")
        daemon.run_forever()
...
def cmd_status(_):
    ...
    console.print(Panel(f"Tasks completed: {count}\nSelf-improve trigger: {mem.should_trigger_self_improve()}", title="ATHENA Memory"))
...
def cmd_bot(_args):
    ...
    banner()
    console.print(Panel(
        "[bold green]Starting ATHENA Bot Orchestrator[/bold green]\n\n"
        "Listening on all configured platforms.\n"
        "Use /goal in chat • /status • /ATHENA on/off • /help\n\n"
        "Logs: logs/bot.log\n"
...
def cmd_doctor(_args):
    ...
    banner()
    console.print("[bold cyan]Running ATHENA Doctor...[/bold cyan]\n")
...
def main():
    parser = argparse.ArgumentParser(description="ATHENA - ATHENA-Hermes Swarm")
    ...
    # ATHENA
    p_ATHENA = sub.add_parser("ATHENA", help="Autonomous background daemon")
    p_ATHENA.add_argument("--once", action="store_true", help="Run one scan cycle then exit")
    p_ATHENA.set_defaults(func=cmd_ATHENA)
