@echo off
REM run_all.bat
REM Comprehensive startup for full ATHENA system with 3D Dashboard
...
if /i "%choice%"=="ATHENA" set "choice=2"
...
if not "%choice%"=="1" if not "%choice%"=="2" if not "%choice%"=="3" (
    ...
    echo 2. ATHENA Daemon (autonomous background scanner)
    echo 3. Full System (Dashboard + ATHENA)
    echo.
...
if "%choice%"=="1" (
    ...
    echo Launching full system: Dashboard + ATHENA...
    start cmd /k "python -m uvicorn backend.dashboard_api:app --reload --reload-dir backend --reload-dir core --reload-dir hermes --port 8001"
    start cmd /k "python -m core.ATHENA_daemon"
    timeout /t 2
    cd dashboard
    if not exist "node_modules" npm install > nul 2>&1
    call npm run dev
