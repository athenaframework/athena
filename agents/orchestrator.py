...
Central swarm coordinator (ATHENA style).
...
class Orchestrator:
    ...
    def run(self, goal: str, max_steps: int = 8) -> AgentResult:
        ...
        emit_agent_update("Oracle", "working", f"Received goal: {goal[:50]}", 20)
        emit_log(f"Swarm started for: {goal[:70]}", "info", "Oracle")

        context = self.memory.get_relevant_context(goal)

        # ATHENA Code style: auto inject @path file references into context
        file_ctx = self.tools.extract_file_context(goal) if hasattr(self.tools, 'extract_file_context') else ""
        if file_ctx:
            context = (context + "\n\n" + file_ctx).strip()

        # Phase 1: Architecture
        emit_agent_update("Nexus", "working", "Analyzing requirements + codebase", 25)
        arch = self.agents["architect"].run(goal, context)
        if not arch.success:
            emit_agent_update("Nexus", "error", "Architecture failed")
            return self._finalize(goal, False, "Architecture phase failed", start)

        # Phase 2: Implementation (may loop internally)
        emit_agent_update("Forge", "working", "Implementing solution using tools", 45)
        impl = self.agents["coder"].run(goal, arch.output + "\n\n" + context)
        if not impl.success:
            emit_agent_update("Forge", "error", "Coding failed")
            return self._finalize(goal, False, "Coding phase failed", start)

        # Phase 3: Testing & verification
        emit_agent_update("Cipher", "working", "Validating implementation", 70)
        test = self.agents["tester"].run(goal, impl.output)
        if not test.success:
            logger.warning("Tests reported issues - continuing to scribe")
            emit_log("Tests had issues but continuing", "warning", "Cipher")

        # Phase 4: Documentation & learning capture
        emit_agent_update("Aegis", "working", "Capturing knowledge + updating soul", 85)
        ...
        emit_agent_update("Oracle", "completed", f"Done in {duration:.1f}s", 100)
        emit_metrics(tasks_completed=self.memory.get_task_count() if hasattr(self.memory, 'get_task_count') else None)
        emit_log(f"Swarm finished. Artifacts: {all_artifacts[:5]}", "success" if success else "warning", "Oracle")
...
def run_swarm(goal: str, project_root: str = ".") -> AgentResult:
    """Convenience top-level function used by main.py and ATHENA."""
