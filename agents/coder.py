...
class Coder:
    name = "coder"
    SYSTEM_PROMPT = """You are an expert senior software engineer in the ATHENA swarm.
...
Produce the complete implementation now. Use the exact FILE: ``` format for every file you create."""
        ...
        emit_agent_update("Forge", "working", "Generating real code from goal...", 50)
        ...
        if created:
            for idx, (path, content) in enumerate(created):
                ...
                if res.get("success"):
                    ...
                    emit_log(f"✓ Wrote real file: {path}", "success", "Forge")
                    emit_agent_update("Forge", "working", f"Created {path}", 50 + (idx * 12))
...
3. Or use `python main.py ATHENA "..."` for the powerful ReAct tool-using agent.
...
"""
        ...
        emit_agent_update("Forge", "completed", f"Real files created: {len(artifacts)}", 95)
        return AgentResult(
    ...
    def _smart_generate(self, goal: str, goal_lower: str, context: str) -> list[tuple[str, str]]:
        ...
        if any(k in goal_lower for k in ["html", "web page", "website", "index.html", "landing page", "hello"]):
            ...
            body = self._extract_body(goal) or "Welcome to the page generated by ATHENA."
    ...
    return (
        <div className="p-8 bg-[#0a0a0f] text-white rounded-2xl border border-[#2a2a35]">
            ...
            <p className="text-[#a0a0b0] mb-6">
                This React component was auto-generated by ATHENA Coder for your goal:
                <br />
...
"""
        ...
        if any(k in goal_lower for k in ["python", "function", "script", "def ", "add two", "calculate", "utility"]):
            ...
            py_code = f'''"""Auto-generated by ATHENA Coder
Goal: {goal}
"""
...
'''
        ...
        if any(k in goal_lower for k in ["api", "fastapi", "endpoint", "route", "server"]):
            ...
            api_code = f'''"""FastAPI endpoint generated by ATHENA
Goal: {goal}
...
app = FastAPI(title="ATHENA Generated API")
...
@app.get("/")
def root():
    return {{"message": "ATHENA API is running", "goal": "{goal[:60]}..."}}
...
'''
        ...
        generic_py = f'''"""Module generated by ATHENA for goal:
{goal}
...
'''
    ...
    def _extract_body(self, goal: str) -> str:
        ...
        return "This page was automatically created by the ATHENA autonomous coding swarm."
