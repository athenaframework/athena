"""bot_orchestrator.py
Central multi-platform bot orchestrator for ATHENA.
...
- Designed so core/ ATHENA/ agents/ are 100% untouched
...
ATHENA_instance: Optional["ATHENADaemon"] = None
ATHENA_running = False
...
def get_status() -> str:
    ...
    try:
        ...
        kstat = "running" if ATHENA_running else "stopped"
        return f"Tasks completed: {count}\nLLM: {prov} / {model}\nATHENA: {kstat}\nUptime: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    except Exception as e:
        return f"Status error: {e}"


async def toggle_ATHENA(on: bool) -> str:
    global ATHENA_instance, ATHENA_running
    if on and not ATHENA_running:
        ATHENA_instance = ATHENADaemon(project_root=str(ROOT))
        # Run in background thread (daemon has its own loop)
        t = threading.Thread(target=ATHENA_instance.run_forever, daemon=True)
        t.start()
        ATHENA_running = True
        return "ATHENA daemon started (background)."
    elif not on and ATHENA_running:
        ATHENA_running = False
        # Note: full stop would require signal in daemon; for v1 we just flag
        return "ATHENA stop requested (will finish current cycle)."
    return "ATHENA already in requested state."
...
def command_router(msg: Message, cmd: BotCommand) -> Optional[str]:
    ...
    if cmd.name == "status":
        return get_status()
    if cmd.name == "help":
        return "Commands: /goal <task> | /status | /ATHENA on|off | /stop"
    if cmd.name == "ATHENA":
...
async def run_bot_forever():
    ...
    logger.info("=== ATHENA Multi-Platform Bot Starting ===")
...
# Gives /status , /trigger_goal , /ATHENA/toggle  (protected by simple token if set)
...
if FastAPI:
    dashboard_app = FastAPI(title="ATHENA Bot Dashboard", version="1.0")
    ...
    async def status():
        return {"status": get_status(), "ATHENA_running": ATHENA_running, "platforms": list(msg_interface.handlers.keys())}
    ...
    @dashboard_app.post("/ATHENA/{action}")
    async def ATHENA_toggle(action: str):
        if action not in ("on", "off"):
            raise HTTPException(400, "Use on or off")
        result = await toggle_ATHENA(action == "on")
