"""core/ATHENA_daemon.py
ATHENA - Autonomous background swarm daemon.
Every N minutes it wakes up, scans the repo for issues, and proposes/fixes small problems
using the full ATHENA stack (ReAct + multi-agent swarm when needed).
All actions respect the safety approval system by default.

Run with:
  python -m core.ATHENA_daemon
  python -m core.ATHENA_daemon --once
  python run_ATHENA.bat
...
logger = logging.getLogger("core.ATHENA")
...
class ATHENADaemon:
    def __init__(self, config_path: str = "config.yaml", project_root: str = "."):
        ...
        logger.info("ATHENA daemon initialized")
    ...
    def _setup_signal_handlers(self):
        def handler(signum, frame):
            logger.info("ATHENA received shutdown signal")
            self.running = False
    ...
    def scan_for_issues(self) -> list[str]:
        ...
        # Dedup + limit
        return list(dict.fromkeys(issues))[: self.config.get("ATHENA", {}).get("max_proactive_fixes", 3)]

    def process_issue(self, issue: str) -> dict:
        """Use the ReAct engine for small autonomous fixes (fast path)."""
        logger.info(f"ATHENA processing issue: {issue}")
        result = self.react.run(goal=f"[ATHENA AUTONOMOUS] {issue}")
    ...
    def run_once(self) -> None:
        ...
        logger.info("ATHENA waking up for scan...")
        issues = self.scan_for_issues()
        if not issues:
            logger.info("ATHENA: No actionable issues found. Sleeping.")
            return

        logger.info(f"ATHENA found {len(issues)} issues")
        for issue in issues:
            if not self.running:
                break
            res = self.process_issue(issue)
            logger.info(f"ATHENA result for '{issue}': success={res.get('success')}")

        # Update soul with autonomous activity
        soul = self.project_root / "ATHENA" / "soul.md"
        with open(soul, "a", encoding="utf-8") as f:
            f.write(f"\n\n## {datetime.now().isoformat()} (ATHENA)\nAutonomous scan completed. Issues addressed: {len(issues)}\n")

    def run_forever(self) -> None:
        interval = self.config.get("ATHENA", {}).get("scan_interval_minutes", 15) * 60
        logger.info(f"ATHENA entering autonomous loop (every {interval//60} minutes)")
        while self.running:
            try:
                self.run_once()
            except Exception as e:
                logger.exception(f"ATHENA cycle error: {e}")
            if self.running:
                time.sleep(interval)


def main():
    parser = argparse.ArgumentParser(description="ATHENA Autonomous Daemon")
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("ATHENA/ATHENA.log", encoding="utf-8"),
        ],
    )

    daemon = ATHENADaemon(project_root=args.root)
    if args.once:
        daemon.run_once()
    else:
        daemon.run_forever()
