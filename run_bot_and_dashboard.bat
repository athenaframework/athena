@echo off
REM ============================================================
REM ATHENA Bot + 3D Dashboard (Unified Launcher)
...
REM 5. Starts ATHENA Dashboard (React) on port 3000 (new window) from the "dashboard" folder
...
echo.
echo ============================================================
echo ATHENA: Bot + Dashboard Unified Launcher
echo ============================================================
echo.
...
start "ATHENA Backend" cmd /k "python -m uvicorn backend.dashboard_api:app --host 0.0.0.0 --port 8001 --reload --reload-dir backend --reload-dir core --reload-dir hermes"

echo ^[2/3^] Starting ATHENA Dashboard (http://localhost:3000)...

set DASHBOARD_DIR=dashboard

REM Auto-install node_modules on first run so 'vite' is available
if not exist "%~dp0\%DASHBOARD_DIR%\node_modules" (
    echo Installing dashboard Node dependencies (first run, takes 1-3 min)...
    pushd "%~dp0\%DASHBOARD_DIR%"
    call npm install --legacy-peer-deps
    popd
)

start "ATHENA Dashboard" cmd /k "cd /d %~dp0\%DASHBOARD_DIR% && npm run dev"
...
REM start "ATHENA Bot" cmd /k "python bot_orchestrator.py"
...
echo.
echo Services:
echo   - ATHENA Dashboard:  http://localhost:3000
...
taskkill /FI "WINDOWTITLE eq ATHENA Backend*" /F >nul 2>&1
taskkill /FI "WINDOWTITLE eq ATHENA Dashboard*" /F >nul 2>&1
REM taskkill /FI "WINDOWTITLE eq ATHENA Bot*" /F >nul 2>&1
